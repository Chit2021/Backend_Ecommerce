...
export async function getAllOffset(req: Request, res: Response) {
  const pageNumber = Number(req.query.pageNumber) || 1;
  const pageSize = Number(req.query.pageSize) || 10;
  const list = await OrdersService.getPaginatedOrder(pageNumber, pageSize)
  res.json({ list });
}


export async function getAllUserOrdersOffset(req: Request, res: Response) {
  const userId = req.params.id
  const pageNumber = Number(req.query.pageNumber) || 1;
  const pageSize = Number(req.query.pageSize) || 10;
  const list = await OrdersService.getPaginatedUserOrder(userId, pageNumber, pageSize)
  res.json({ list });
}
